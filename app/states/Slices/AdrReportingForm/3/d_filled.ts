...
import {
  ...
  ActionType,
  ActionType1,
  FormStateTypeAdditional,
} from "~/types/reducers/adrReporting/3/d_filled";
...
export const form1page3hFilledSlice = createSlice({
  ...
  reducers: {
    setAdditionalFormData: (state, action: PayloadAction<ActionType>) => {
      ...
      state.drugDetails = [
        ...state.drugDetails,
        { key: action.payload.id, ...action.payload.drugDetails },
      ];
    },
    removeAdditionalFormData: (state, action: PayloadAction<ActionType1>) => {
      ...
      state.drugDetails = state.drugDetails?.filter(
        (item: any) => item.key !== action.payload.id
      );
    },
...
} = form1page3hFilledSlice.actions;
