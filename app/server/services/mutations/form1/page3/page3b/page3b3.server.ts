...
const addToForm = async (user: string, formID: string, formInput: any) => {
    try {
        ...
        if (form1Pointer) {
            ...
            form1Pointer.form1Page3.Form1Page3b.isComplete = true;
            form1Pointer.form1Page3.Form1Page3b.form1page3b3 = { ...formInput, isComplete: true }
            form1Pointer.save()
                .then(() => {
                    console.log("Form 1 page 3b3 updated")
                }
                ).catch(error => {
                    console.log(error)
                })
            return "Form 1 Page 3b3 modified successfully"
        } else {
...
const getForm = async (user: string, formID: string) => {
    try {
        ...
        if (form1Pointer) {
            ...
            return data?.Form1Page3b?.form1page3b3
        } else {
            return "Form 1 Page 3b3 does not exist"
        }
    }
    catch (error) {
    }
