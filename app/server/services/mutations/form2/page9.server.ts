...
const addToForm = async (user: string, formID: string, formInput: any) => {
    try {
        ...
        if (form2Pointer) {
            // update form2.form2Page9 based on formInput
            const sanitizedFormInput = { ...formInput, isComplete: true };
            form2Pointer.form2Page9 = sanitizedFormInput;
            form2Pointer.save().then(() => {
                console.log("Form 2 page 9 updated")
            }
            ).catch(error => {
                console.log(error)
            })
            return "Form 2 Page 9 modified successfully"
        } else {
...
const getForm = async (user: string, formID: string) => {
    try {
        const form2Pointer = await Forms2Model.findOne({ user: user, _id: formID })
        if (form2Pointer) {
            return form2Pointer.form2Page9
        } else {
            return "Form 2 Page 9 does not exist"
        }
    }
    catch (error) {
    }
